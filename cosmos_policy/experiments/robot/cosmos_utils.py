...
def _read_packnet_meta(packnet_path: str) -> dict:
    ...
    meta_path = os.path.join(_resolve_dcp_checkpoint_dir(path), _DCP_META_FILE)
    meta = torch.load(meta_path, map_location="cpu", weights_only=False)
    return {
        "prune_perc": float(meta.get("prune_perc", 0.25)),
        "task_id": int(meta.get("task_id", 0)),
    }


class PackNetEvalState:
    """PackNet masks + pristine checkpoint weights for per-task eval masking."""

    def __init__(self, packnet: PackNet, weight_backup: Dict[str, torch.Tensor]) -> None:
        self.packnet = packnet
        self.weight_backup = weight_backup


def packnet_eval_task_id_for_libero_task(libero_task_id: int, first_cl_libero_task: int = 6) -> int:
    """Map LIBERO task index → PackNet ``eval_task_id`` (base tasks 0..first_cl-1 → 0)."""
    if libero_task_id < first_cl_libero_task:
        return 0
    return libero_task_id - first_cl_libero_task + 1


def load_packnet_eval_state(model, packnet_mask_path: str) -> PackNetEvalState:
    """Load PackNet masks and snapshot prunable weights before any eval masking."""
    meta = _read_packnet_meta(packnet_mask_path)
    packnet = PackNet(
        prune_perc=meta["prune_perc"],
        task_id=meta["task_id"],
    )
    packnet.load(packnet_mask_path, net=model.net)

    name_to_param = dict(model.net.named_parameters())
    backup: Dict[str, torch.Tensor] = {}
    for name in packnet.masks:
        if name in name_to_param:
            backup[name] = _full_tensor(name_to_param[name].data).detach().cpu().clone()

    print(
        f"[PackNet] Loaded masks from {packnet_mask_path} "
        f"(task_id={packnet.task_id}, prune_perc={packnet.prune_perc}, layers={len(backup)})"
    )
    return PackNetEvalState(packnet=packnet, weight_backup=backup)


def apply_packnet_eval_for_libero_task(
    model,
    packnet_eval_state: PackNetEvalState,
    libero_task_id: int,
    first_cl_libero_task: int = 6,
) -> int:
    """Restore checkpoint weights, then apply PackNet eval mask for ``libero_task_id``."""
    eval_task_id = packnet_eval_task_id_for_libero_task(libero_task_id, first_cl_libero_task)
    name_to_param = dict(model.net.named_parameters())
    for name, full_w in packnet_eval_state.weight_backup.items():
        param = name_to_param.get(name)
        if param is not None:
            _copy_full_weight_into_param(param, full_w.to(param.device))
    packnet_eval_state.packnet.apply_eval_mask(model.net, eval_task_id=eval_task_id)
    return eval_task_id
...
def get_model(cfg):
    ...
    packnet_mask_path = getattr(cfg, "packnet_mask_path", "") or ""
    if packnet_mask_path:
        model._packnet_eval_state = load_packnet_eval_state(model, packnet_mask_path)
    else:
        model._packnet_eval_state = None
...
def extract_proprio_from_latent_sequence(
    output_latent: torch.Tensor, proprio_dim: int, proprio_indices: torch.Tensor
) -> torch.Tensor:
    """
    Extract predicted proprio from the generated latent sequence.

    This mirrors action-chunk extraction:
    flatten the proprio latent frame, split into full proprio vectors, and average
    across all full vectors to get a robust final proprio prediction.
    """
    batch_indices = torch.arange(output_latent.shape[0], device=output_latent.device)
    proprio_latent_frame = output_latent[batch_indices, :, proprio_indices, :, :]  # (B, C', H', W')
    batch_size = proprio_latent_frame.shape[0]
    flat_proprio_latent = proprio_latent_frame.reshape(batch_size, -1)
    num_latent_elements = flat_proprio_latent.shape[1]
    if proprio_dim > flat_proprio_latent.shape[1]:
        raise ValueError(
            f"proprio_dim={proprio_dim} exceeds available latent elements={flat_proprio_latent.shape[1]}"
        )
    if proprio_dim < 1:
        raise ValueError(f"proprio_dim must be >= 1, got {proprio_dim}")
    # Calculate how many full proprio vectors we can extract
    num_proprio_chunks = num_latent_elements // proprio_dim
    # Keep only full vectors and reshape
    # New shape: (batch_size, num_proprio_chunks, proprio_dim)
    all_proprio_chunks = flat_proprio_latent[:, : num_proprio_chunks * proprio_dim].reshape(
        batch_size, num_proprio_chunks, proprio_dim
    )
    # Average across duplicated vectors
    # New shape: (batch_size, proprio_dim)
    final_proprio = torch.mean(all_proprio_chunks, dim=1)
    return final_proprio
...
def unnormalize_future_proprio(proprio: np.ndarray, dataset_stats: dict, scale_multiplier: float = 1.0):
    """
    Undo proprio normalization back to original dataset scale.

    Uses ``future_proprio_*`` stats when present, otherwise falls back to ``proprio_*``.
    """
    proprio_min = dataset_stats.get("future_proprio_min", dataset_stats["proprio_min"])
    proprio_max = dataset_stats.get("future_proprio_max", dataset_stats["proprio_max"])
    proprio = proprio / scale_multiplier
    proprio = 0.5 * (proprio + 1) * (proprio_max - proprio_min) + proprio_min
    return proprio
...
def get_action(
    ...
    with torch.inference_mode():
        ...
        if batch_size > 1:
            ...
            if generate_future_state_and_value_in_parallel:
                future_proprio_indices = torch.full(
                    (batch_size,),
                    future_proprio_latent_idx,
                    dtype=torch.int64,
                    device=generated_latent_with_action.device,
                )
                future_proprio_prediction = (
                    extract_proprio_from_latent_sequence(
                ...
                if cfg.normalize_proprio:
                    future_proprio_prediction = unnormalize_future_proprio(future_proprio_prediction, dataset_stats)
        ...
        if generate_future_state_and_value_in_parallel:
            ...
            return_dict["future_proprio_prediction"] = future_proprio_prediction
    ...
    with torch.inference_mode():
        ...
        if batch_size > 1:
            ...
            future_proprio_indices = torch.full(
                    ...
                    future_proprio_latent_idx,
                    dtype=torch.int64,
                    device=generated_latent_with_future_state.device,
                )
              
            future_proprio_prediction = (
                extract_proprio_from_latent_sequence(
                    generated_latent_with_future_state.clone(),
                    proprio_dim=data_batch["proprio"].shape[-1],
                    proprio_indices=future_proprio_indices,
                )
                .to(torch.float32)
                .cpu()
        ...
        return_dict = dict(
            ...
            future_proprio_prediction=future_proprio_prediction,
        )
