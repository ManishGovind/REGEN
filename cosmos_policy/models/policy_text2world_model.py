...
from cosmos_policy.continual.ewc import OnlineEWC, parse_dtype
...
class CosmosPolicyModelConfig(BaseText2WorldModelConfig):
    ...
    # ----------------------------------------------------------------------
    # Online Elastic Weight Consolidation (EWC) for continual learning.
    #
    # EWC adds a quadratic penalty
    #     L_ewc = (lambda / 2) * sum_i F_i (theta_i - theta*_i)**2
    # to the regular training loss to prevent forgetting prior CL stages. The
    # online variant maintains a single running Fisher diagonal `F` and a
    # single parameter snapshot `theta*`, updated after each task as
    #     F      <-  gamma * F + F_new
    #     theta* <-  theta_new
    # See cosmos_policy/continual/ewc.py for the implementation.
    # ----------------------------------------------------------------------
    ewc_enabled: bool = False  # Master switch -- when False, the EWC code path is bypassed entirely.
    # Defaults below mirror continual-openpi's TrainConfig (libero-sequential):
    #   src/openpi/training/config.py -> ewc_lambda=50000, ewc_gamma=0.9, ewc_max_batches=50
    # which is the closest published analog (VLA + LIBERO + one task per stage).
    ewc_lambda: float = 50000.0  # Regularisation strength.
    ewc_gamma: float = 0.9  # Decay applied to the running Fisher (1.0 = canonical EWC, <1.0 = exponential decay of old tasks).
    ewc_prev_state_path: Optional[str] = None  # DCP directory (meta.pt + ewc_state/) or legacy .pt; loaded after checkpoint.
    ewc_save_state_path: Optional[str] = None  # DCP directory or legacy .pt; written at end of training after compute_fisher.
    ewc_num_fisher_batches: int = 50  # How many batches to use for the Fisher estimate at end of training.
    # Storage dtype for Fisher / theta* tensors.
    # "auto" (default) => match each parameter's own dtype, so the EWC state
    # inherits the base-stage checkpoint precision exactly. Set to e.g.
    # "bfloat16" or "float32" only if you explicitly want to override.
    ewc_fisher_dtype: str = "auto"
    ewc_log_every: int = 5  # Log the EWC penalty every N optimiser steps (0 = never).

    # ----------------------------------------------------------------------
    # PackNet (Mallya & Lazebnik, 2018) for continual learning.
    #
    # After each CL stage: prune lowest-magnitude weights among the current
    # task allocation, optional post-prune fine-tuning, then freeze via masks.
    # See cosmos_policy/continual/packnet.py and continual/PACKNET.md.
    # ----------------------------------------------------------------------
    packnet_enabled: bool = False
    packnet_task_id: int = 0  # Zero-based PackNet task index for *this* CL stage (base = 0, first CL = 1, …).
    packnet_prune_perc: float = 0.25  # Fraction of current-task weights pruned after training.
    packnet_prev_state_path: Optional[str] = None  # DCP directory or legacy .pt from the previous stage.
    packnet_save_state_path: Optional[str] = None  # Written after prune (+ post-prune finetune if any).
    # Masks matching the last training checkpoint (before end-of-stage prune). Use for LIBERO eval
    # with iter_N ckpts; defaults to ``{packnet_save_state_path}_pre_prune`` when unset.
    packnet_save_pre_prune_state_path: Optional[str] = None
    # When True and packnet_prev_state_path is unset: loaded checkpoint = base task 0 (already trained);
    # prune 25% of base weights at startup, then train only the freed slots for packnet_task_id >= 1.
    packnet_init_from_base: bool = False
    packnet_base_task_id: int = 0  # Task index assigned to the pretrained base checkpoint (usually 0).
    # Post-prune fine-tuning iterations (0 = skip). LIBERO uses the same count as training epochs (e.g. 50).
    packnet_post_prune_iters: int = 0
    # Save a training checkpoint after post-prune fine-tune (iter = main training iter + post_prune_iters).
    packnet_save_post_prune_ckpt: bool = True
    # Iteration folder for the base post-prune retrain checkpoint (default 1 avoids colliding with
    # save_iter=1000 during main CL training when base retrain also runs 1000 iters).
    packnet_base_post_prune_ckpt_iter: int = 1
    # After init_from_pretrained_base: retrain the pruned base subnet on prior-task data before CL training.
    packnet_base_post_prune_iters: int = 0
    packnet_log_every: int = 0  # Reserved for future PackNet-specific logging (0 = off).
    ...
    def __attrs_post_init__(self):
        ...
        assert not (self.ewc_enabled and self.packnet_enabled), (
            "Enable either EWC (ewc_enabled) or PackNet (packnet_enabled), not both."
        )
...
class CosmosPolicyDiffusionModel(BaseDiffusionModel):
    ...
    def __init__(self, config: CosmosPolicyModelConfig):
        ...
        # Online EWC helper. Always constructed so that downstream code can
        # interrogate `model.ewc.has_state()` cheaply, but it stays a no-op
        # until either `compute_fisher` or `load` is called.
        if config.ewc_enabled:
            self.ewc = OnlineEWC(
                lambda_=config.ewc_lambda,
                gamma=config.ewc_gamma,
                fisher_dtype=parse_dtype(config.ewc_fisher_dtype),
            )
        else:
            self.ewc = None  # type: ignore[assignment]

        if config.packnet_enabled:
            self.packnet = PackNet(
                prune_perc=config.packnet_prune_perc,
                task_id=config.packnet_task_id,
            )
        else:
            self.packnet = None  # type: ignore[assignment]

    def on_train_start(self, memory_format: torch.memory_format = torch.preserve_format) -> None:
        """Extends base hook; EWC / PackNet state is loaded after the checkpoint (see below)."""
        super().on_train_start(memory_format)
        if self.packnet is not None:
            self.packnet.set_norm_layers_eval(self.net)

    def load_ewc_state_if_configured(self) -> None:
        """Load Fisher / theta* after the model checkpoint has been restored.

        Must run **after** ``checkpointer.load`` so ``net``'s FSDP shard layout
        matches the tensors we re-shard from disk. Loading earlier (e.g. in
        ``on_train_start``) can leave theta* with the wrong local shard shapes.
        """
        if self.ewc is None:
            return
        prev_state_path = self.config.ewc_prev_state_path
        if not prev_state_path:
            if distributed.is_rank0():
                log.warning(
                    "[EWC] ewc_enabled=True but no ewc_prev_state_path provided -- "
                    "training without an EWC penalty for this stage. Fisher will still be "
                    "computed at end of training if ewc_save_state_path is set."
                )
            return

        log.critical(f"[EWC] Loading prior task state from: {prev_state_path}")
        # Always let this run's config win for lambda/gamma; the values stored
        # in the file are only metadata (e.g. compute_ewc_fisher.py defaults
        # to lambda=gamma=1.0 because they don't affect Fisher estimation).
        self.ewc.load(
            prev_state_path,
            net=self.net,
            override_lambda=self.config.ewc_lambda,
            override_gamma=self.config.ewc_gamma,
        )
        self.ewc.to("cuda")

    def load_packnet_state_if_configured(self) -> None:
        """Load PackNet masks after the model checkpoint has been restored."""
        if self.packnet is None:
            return

        prev_path = self.config.packnet_prev_state_path
        if prev_path:
            log.critical(f"[PackNet] Loading prior masks from: {prev_path}")
            self.packnet.load(prev_path, net=self.net)
            self.packnet.start_task(self.net)
        elif self.config.packnet_init_from_base:
            self.packnet.init_from_pretrained_base(
                self.net,
                base_task_id=self.config.packnet_base_task_id,
            )
        else:
            if distributed.is_rank0():
                log.info(
                    "[PackNet] No packnet_prev_state_path and packnet_init_from_base=False -- "
                    "standard sequential PackNet (all free weights assigned to this task)."
                )
            self.packnet.init_masks(self.net)
            self.packnet.start_task(self.net)
        self.packnet.to("cuda")
        self.packnet.set_norm_layers_eval(self.net)

    def on_after_backward(self, iteration: int = 0) -> None:
        if self.packnet is not None:
            self.packnet.zero_grads(self.net)
        super().on_after_backward(iteration=iteration)
    ...
    def training_step(
        ...
        *,
        ...
        Args:
            ...
            include_ewc_penalty (bool): If False, return task loss only (for Fisher estimation).
        ...
        # ------------------------------------------------------------------
        # Online EWC penalty: lambda/2 * sum_i F_i (theta_i - theta*_i)^2.
        # Computed on the LOCAL shard for each rank (FSDP correct-by-construction;
        # see cosmos_policy/continual/ewc.py for the analysis).
        # ------------------------------------------------------------------
        if include_ewc_penalty and self.ewc is not None and self.ewc.has_state():
            ewc_penalty = self.ewc.penalty(self.net.named_parameters())
            output_batch["ewc_penalty"] = ewc_penalty.detach()
            kendall_loss = kendall_loss + ewc_penalty
            log_every = self.config.ewc_log_every
            if (
                log_every
                and iteration % log_every == 0
                and distributed.is_rank0()
            ):
                log.info(
                    f"[EWC] iter={iteration} kendall_loss={kendall_loss.detach().item():.6g} local_penalty={ewc_penalty.detach().item():.6g} "
