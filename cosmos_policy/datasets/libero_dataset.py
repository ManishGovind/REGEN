...
from cosmos_policy.datasets.dataset_utils import (
    ...
    get_hdf5_files,
    instruction_from_hdf5_filename,
    get_instruction_from_episode,
    preprocess_image,
    rescale_data,
...
class LIBERODataset(Dataset):
    def __init__(
        ...
        gamma: float = 0.99,
        max_replay_demos: Optional[int] = None,
        replay_tasks: Optional[Sequence[str]] = None,
        current_tasks_ids: Optional[Sequence[int]] = None,
    ):
        """
        ...
        Args:
            ...
            include_only_task_instructions (Optional[Sequence[str]]): If set, load only demonstration HDF5s whose
                filename-derived instruction matches one of these strings (case- and surrounding-whitespace-insensitive),
                and only rollout episodes whose ``task_description`` attribute matches. When None, all tasks are kept.
            max_replay_demos (Optional[int]): Maximum expert demos per task HDF5 (first N after sorting
                ``demo_*`` keys). When ``er_cap_previous_task_instructions`` is unset, this cap applies to **every**
                demonstration file (if set). When ``er_cap_previous_task_instructions`` is set, this cap applies **only**
                to files whose filename-derived instruction matches one of those strings; **current** tasks load all demos.
            er_cap_previous_task_instructions (Optional[Sequence[str]]): Task instructions (same normalization as
                ``include_only_task_instructions``) for which ER-style demo capping applies. Requires
                ``max_expert_demos_per_task_file`` to be set. When None or empty, behavior reverts to a global cap via
        ...
        self.er_rollout_data_dir = er_rollout_data_dir
        ...
        self.gamma = gamma
        self.current_tasks_ids = current_tasks_ids
    
        if max_replay_demos is not None:
            if not isinstance(max_replay_demos, int) or max_replay_demos < 1:
                raise ValueError(
                    "max_expert_demos_per_task_file must be None or a positive int, "
                    f"got {max_replay_demos!r}"
                )
        self.max_replay_demos = max_replay_demos

        if replay_tasks is not None:
            _er_prev = {s.strip().lower() for s in replay_tasks if s.strip()}
            self.replay_tasks = _er_prev if _er_prev else None
        else:
            self.replay_tasks = None
        if self.replay_tasks is not None and self.max_replay_demos is None:
            raise ValueError(
                "max_expert_demos_per_task_file must be set when er_cap_previous_task_instructions is non-empty "
                "(ER-style cap on previous tasks only)."
            )

        assert self.use_wrist_images or self.use_third_person_images, (
            "Must use at least one of wrist images or third-person images!"
        )
        if self.current_tasks_ids is not None:
            self.current_tasks_ids = [str(id) for id in self.current_tasks_ids]
        ...
        if self.er_data_dir != "":
            ...
            if self.max_replay_demos is not None:
                er_files_by_instruction = defaultdict(list)
                for er_file in er_hdf5_files:
                    er_instruction = instruction_from_hdf5_filename(er_file).strip().lower()
                    er_files_by_instruction[er_instruction].append(er_file)
                capped_er_hdf5_files = []
                for _, files_for_instruction in er_files_by_instruction.items():
                    # Keep ordering deterministic; filenames are typically ..._demo{N}.hdf5
                    sorted_files = sorted(files_for_instruction)
                    capped_er_hdf5_files.extend(sorted_files[: self.max_replay_demos])
                er_hdf5_files = capped_er_hdf5_files
        else:
            er_hdf5_files = []

        if self.current_tasks_ids is not None:
            hdf5_files = [file for file in hdf5_files if instruction_from_hdf5_filename(file) in self.current_tasks_ids] 
        hdf5_files = hdf5_files + er_hdf5_files    
        print(f"Total number of Demos to policy train on: {len(hdf5_files)}")
        ...
        if self.rollout_data_dir:
            ...
            if self.current_tasks_ids is not None:
                rollout_hdf5_files_current_tasks = [file for file in rollout_hdf5_files if get_instruction_from_episode(file) in self.current_tasks_ids]
            capped_rollout_hdf5_files = []
            if self.replay_tasks is not None:
                rollout_hdf5_files_replay_tasks = [
                    file for file in rollout_hdf5_files if get_instruction_from_episode(file) in self.replay_tasks
                ]
                rollout_files_by_instruction = defaultdict(list)
                for file in rollout_hdf5_files_replay_tasks:
                    instruction = get_instruction_from_episode(file).strip().lower()
                    rollout_files_by_instruction[instruction].append(file)

                for _, files_for_instruction in rollout_files_by_instruction.items():
                    # Keep ordering deterministic; filenames are typically ..._demo{N}.hdf5
                    sorted_files = sorted(files_for_instruction)
                    capped_rollout_hdf5_files.extend(sorted_files[: self.max_replay_demos])   
            rollout_hdf5_files = rollout_hdf5_files_current_tasks + capped_rollout_hdf5_files
        print(f"Total number of Rollouts to policy train on: {len(rollout_hdf5_files)}")
        ...
        if self.demonstration_sampling_prob > 0:  # Only load demos if they are used
            for file in tqdm(hdf5_files):
                with h5py.File(file, "r") as f:
                    ...
                    command = instruction_from_hdf5_filename(file)
                    file_instruction_lc = command.strip().lower()
                    
                    if self.max_replay_demos is not None and self.replay_tasks is not None:
                        if file_instruction_lc in self.replay_tasks:
                            sorted_demo_keys = sorted_demo_keys[: self.max_replay_demos]
        ...
        if len(rollout_hdf5_files) > 0:
            for file in tqdm(rollout_hdf5_files, desc="Loading rollout metadata"):
                with h5py.File(file, "r") as f:
                    ...
                    # if self.current_tasks_ids is not None:
                    #     if command not in self.current_tasks_ids :
                    #         continue
                    self.unique_commands.add(command)
                    # Get success flag
            ...
            print(f"Total number of Rollouts to policy train on: {len(self.rollout_episode_metadata)}")
