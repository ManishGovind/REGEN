...
def instruction_from_hdf5_filename(hdf5_path: str) -> str:
    """Language instruction inferred from a LIBERO-style demo HDF5 basename (matches ``LIBERODataset``).

    Strips a ``*_demo.hdf5`` or ``*_demoN.hdf5`` suffix (N integer, e.g. ``demo0``, ``demo1``) when present;
    otherwise uses the full basename without extension.
    """
    raw = os.path.basename(hdf5_path)
    m = re.match(r"^(.*)_demo(\d+)?\.hdf5$", raw, flags=re.IGNORECASE)
    base = m.group(1) if m else os.path.splitext(raw)[0]
    words = base.split("_")
    command = ""
    for w in words:
        if "SCENE" in w:
...
def task_description_from_all_episodes_filename(hdf5_path: str) -> Optional[str]:
    """Infer LIBERO task description from all_episodes filename.

    Expected filename pattern:
    ``episode_data--suite=<suite>--...--task=<task_id>--...hdf5``
    """
    base = os.path.basename(hdf5_path)
    match = re.search(r"suite=(libero_[^-\s]+)--.*--task=(\d+)--", base)
    if not match:
        return None
    ...
    # Local import to avoid introducing config-module import overhead at package load time.
    from cosmos_policy.config.experiment.libero_task_constants import LIBERO_SUITE_TASK_ID_TO_DESCRIPTION

    suite_map = LIBERO_SUITE_TASK_ID_TO_DESCRIPTION.get(suite_name)
    if suite_map is None:
        return None
    task_description = suite_map.get(task_id)
    if task_description is None:
        return None
    return task_description.replace("_", " ")

def get_instruction_from_episode(episode_path: str) -> str:
...
def calculate_dataset_statistics(data):
    ...
    stats = {
        ...
        "future_proprio_min": proprio_min,
        "future_proprio_max": proprio_max,
        "future_proprio_mean": proprio_mean,
        "future_proprio_std": proprio_std,
        "future_proprio_median": proprio_median,
    }
